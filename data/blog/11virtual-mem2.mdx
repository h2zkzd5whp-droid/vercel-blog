...
1. read/write가 MMU의 TLB로 감
2. TLB가 주소의 페이지 번호로 조회
3. 페이지 번호 매칭 → PTE 반환
4. TLB가 PTE의 protection이 read/write 허용하는지 검증 (권한 체크)
5. PTE가 어느 물리 프레임인지 지정
6. MMU가 프레임 + offset 합쳐 물리주소 만듦
7. MMU가 그 물리주소에서 읽어 CPU에 값 반환
