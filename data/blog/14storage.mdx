...
tags: ['OS']
draft: false
summary: 'secondary storage, HDD structure, disk performance, disk scheduling algorithms'
---

- [1. Secondary Storage (여기서는 주로 HDD를 다룬다)](#1-secondary-storage-여기서는-주로-hdd를-다룬다)
- [2. HDD의 구조](#2-hdd의-구조)
  - [1. Mechanical (느림의 근원 = 물리적으로 움직이는 부품)](#1-mechanical-느림의-근원--물리적으로-움직이는-부품)
  - [2. Electronics](#2-electronics)
  - [3. 물리 단위](#3-물리-단위)
- [3. OS가 HDD를 어떻게 다루는지 (Managing Disks)](#3-os가-hdd를-어떻게-다루는지-managing-disks)
  - [1. 접근 추상화 계층 (위=추상적, 아래=물리적)](#1-접근-추상화-계층-위추상적-아래물리적)
  - [2. 고수준 인터페이스 (예: SCSI)](#2-고수준-인터페이스-예-scsi)
- [4. Disk Performance 3요소](#4-disk-performance-3요소)
  - [1. Seek time](#1-seek-time)
  - [2. Rotational latency](#2-rotational-latency)
  - [3. Transfer time](#3-transfer-time)
- [5. Seek 병목을 줄이는 방법 (Disk Scheduling)](#5-seek-병목을-줄이는-방법-disk-scheduling)
  - [1. 목적](#1-목적)
  - [2. 알고리즘](#2-알고리즘)
- [6. HDD 사용 실전 적용](#6-hdd-사용-실전-적용)
  - [1. 알고리즘 선택 기준](#1-알고리즘-선택-기준)
  - [2. I/O Scheduler가 해야 하는 일](#2-io-scheduler가-해야-하는-일)
  - [3. 요즘 disk (Modern Disks)](#3-요즘-disk-modern-disks)
...
![HDD-structure](/static/images/osImg/14hdd-structure.png)
