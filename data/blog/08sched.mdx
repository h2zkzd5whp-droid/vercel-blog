...
- [1. CPU scheduling의 목적](#1-cpu-scheduling의-목적)
- [2. 스케줄링의 목적](#2-스케줄링의-목적)
  - [1. starvation이 없어야한다](#1-starvation이-없어야한다)
  - [2. batch system](#2-batch-system)
  - [3. Interactive system](#3-interactive-system)
  - [4. Real-time system](#4-real-time-system)
- [3. 기본개념](#3-기본개념)
  - [1. non-preemptive vs preemptive](#1-non-preemptive-vs-preemptive)
  - [2. cpu-bound vs I/O-bound](#2-cpu-bound-vs-io-bound)
- [4. state queue 흐름](#4-state-queue-흐름)
- [5. scheduling 알고리즘 4가지](#5-scheduling-알고리즘-4가지)
  - [1. FCFS/FIFO](#1-fcfsfifo)
  - [2. SJF(Shortest Job First)](#2-sjfshortest-job-first)
  - [3. SRTF(Shortest Remaining Time First)](#3-srtfshortest-remaining-time-first)
  - [4. RR(Round Robin)](#4-rrround-robin)
- [6. Priority Scheduling](#6-priority-scheduling)
  - [Priority Scheduling의 문제점](#priority-scheduling의-문제점)
    - [1. Starvation](#1-starvation)
    - [2. Priority inversion(우선순위 역전)](#2-priority-inversion우선순위-역전)
- [7. Multilevel Queue Scheduling](#7-multilevel-queue-scheduling)
  - [foreground](#foreground)
  - [background](#background)
  - [queue간의 스케줄링](#queue간의-스케줄링)
- [8. MLFQ(Multilevel Feedback Queue)](#8-mlfq멀티레벨-피드백-큐)
  - [multilevel queue와의 차이점](#multilevel-queue와의-차이점)
  - [흐름](#흐름)
- [9. UNIX 스케줄러](#9-unix-스케줄러)
- [10. 멀티프로세서에서 스케줄랑](#10-멀티프로세서에서-스케줄랑)
- [11. NUMA(Non-Uniform Memory Access)](#11-numanon-uniform-memory-access)
...
- 단점: waiting time이 길어질 수 있다(cpu burst가 긴 프로세스가 먼저 도착하면 아무리 짧은 프로세스가 나중에 도착해면 긴 프로세스를 먼저 실행하기 때문에), CPU가 중간에 길게 I/O사용하면 문제가 된다
  ![FCFS diagram](/static/images/osImg/OS-08-sched-FCFS.png)
...
- 문제점: 미래의 CPU burst time를 알 수 없다(그래서 과거 패턴보고 예측함), starvation이 발생 가능
  ![SJF diagram](/static/images/osImg/OS-08-sched-SJF.png)
...
- SRTF는 T2가 도착했을 때 두개의 burst time를 비교해서(T1:3, T2:2) T2가 더 짧네? T2로 전환이 가능. 왜냐 preemptive이기 때문에.
  ![SRTF diagram](/static/images/osImg/OS-08-sched-SRTF.png)
...
- preemptive 방식
  ![RR diagram](/static/images/osImg/OS-08-sched-RR.png)
